...
if ($selectedComputers.Count -eq 0) {
    ...
    exit
} elseif ($HILSelectionResult -ne $true -or $selfTestVersionResult -ne $true) {
	Write-Host "Exiting..."
	exit
} else {
	Write-Host "Selected computers: $($selectedComputers -join ', ')"
...
function Create-iniFile {
	...
	Set-Content "\\$computerName\C$\ETASData\G5HIL\TCMSelfTest\TCMSelfTest.ini" 'VERSION=$selfTestVersion n\ ACROMAG_INTEGRATION_VERSION_REQUIRED=$requiredAcromagIntegrationVersion'
	if (Test-Path -Path ".\TCMSelfTestCS\TCMSelfTestCS\bin\release") {
